<script>
	...
	let sharedContents = Object.entries(company.contents).reverse();
	...
	async function newContent() {
		...
		const res = await fetch(`${env.PUBLIC_API_URL}/company/content`, {
			method: 'POST',
			headers: {
	...
	async function deleteContent(id) {
		const res = await fetch(`${env.PUBLIC_API_URL}/company/content/${company.companyname}/${id}`, {
			method: 'DELETE',
			headers: {
				Authorization: `${Cookies.get('token')}`
			}
	...
	<div class="w-1/3">
		<div class="bg-gitcol-900 rounded-lg w-full py-2 space-y-2 flex flex-col items-center">
			<img
				...
				src={`${env.PUBLIC_API_URL}/file/${company.profilePicture}`}
				alt={company.companyname}
			/>
			<div class="text-white text-2xl whitespace-nowrap px-1 w-fit">{company.companyname}</div>
			{#if company.users.find((user) => user.username === $userStore.username && user.role === 1)}
				<button
					class="bg-gitcol-600 rounded-lg w-2/3 py-2 transition-all hover:bg-gitcol-500 text-white text-sm whitespace-nowrap"
					on:click={() => {
						goto('/company/' + data.company.companyname + '/edit');
					}}
				>
					Şirket'i Düzenle
				</button>
