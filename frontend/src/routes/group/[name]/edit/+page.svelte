<script>
	...
	let newData = {
		newGroupname: data.group ? data.group.username : '',
		profilePicture: data.group ? `${env.PUBLIC_API_URL}/file/${data.group.profilePicture}` : ''
	};

	onMount(() => {
		if ($userStore.username) {
			const finduser = data.group.users.find(
				(user) => user.username === $userStore.username && user.role
			);

			if (!finduser) {
				goto(`/group/${data.group.username}`);
			}
		} else {
			goto(`/profile/${data.group.username}`);
		}
	});
	...
	async function editGroup() {
		...
		formData.append('newGroupname', newData.newGroupname);

		let response = await fetch(`${env.PUBLIC_API_URL}/group/${data.group.username}`, {
			method: 'PATCH',
			headers: {
		...
		if (response.ok) {
			goto(`/group/${newData.newGroupname}`);
		}
	}
	...
	async function addUser() {
		...
		let response = await fetch(`${env.PUBLIC_API_URL}/group/${data.group.username}/user`, {
			method: 'POST',
			headers: {
				Authorization: `${Cookies.get('token')}`,
...
<main
	class="space-x-5 bg-gitcol-800 rounded-lg border space-y-5 border-gitcol-600 rounded-t-lg p-5"
>
	<div class="flex space-x-5 items-center flex-col">
			...
			<div>
				...
				<div class="profilepic m-4 cursor-pointer rounded-full" on:click={openFileInput}>
					...
					<div class="profilepic__content">
						...
						<span class="profilepic__text">Edit Group</span>
					</div>
				</div>
				<input
					type="file"
				...
				<div class="w-full max-w-lg">
					<div class="flex flex-wrap -mx-3 mb-6">
						<div class="w-full px-3 mb-6 md:mb-0">
							...
							<input
								type="text"
								bind:value={newData.newGroupname}
								name="groupname"
								placeholder={data.group.username}
		...
		<button
			...
			on:click={() => editGroup()}>Kaydet</button
		>
	</div>
	<div class="flex items-center space-x-5 flex-col">
		<div class="flex space-x-5 items-center">
			<div class="w-96">
				<div class="w-full max-w-lg">
					<div class="flex flex-wrap -mx-3 mb-6">
